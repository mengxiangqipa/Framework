    ...
    public static StringRequest.Builder getBuilder(boolean forceBuild) {
        ...
        if (builder == null) {
            synchronized (StringRequest.class) {
                if (builder == null) {
                    builder = new StringRequest.Builder().addHeader("biuoo-from", "android").
                            addHeader(ConstantsME.token, PreferencesHelper.getInstance().getStringData(ConstantsME.token))
