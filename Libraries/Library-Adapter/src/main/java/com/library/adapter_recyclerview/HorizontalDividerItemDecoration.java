...
public class HorizontalDividerItemDecoration extends RecyclerView.ItemDecoration
{
    ...
    public void getItemOffsets(Rect outRect, int itemPosition, RecyclerView parent)
    {
        ...
        {
            if (itemPosition < ((UniversalAdapter) parent.getAdapter()).getHeaderCount() || itemPosition >= ((UniversalAdapter) parent.getAdapter()).getDataItemCount() - 1)
            {
    ...
    public void onDraw(Canvas c, RecyclerView parent, RecyclerView.State state)
    {
        if (parent.getAdapter() instanceof UniversalAdapter)
        {
            ...
            for (int i = 0; i < childCount; i++)
            {
                ...
                if (adapter.isHeader(position) || adapter.isFooter(position) || position == (adapter.getHeaderCount() + adapter.getDataItemCount() - 1))
                    continue;
