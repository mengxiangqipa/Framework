		...
		public void onScrolled(RecyclerView recyclerView, int dx, int dy)
		{
			if (closeOnAction && (dx != 0 || dy != 0))
			{
				...
				if (openedPositions.size() > 0)
				{
					...
					{
						RecyclerView.ViewHolder holder = recyclerView.findViewHolderForAdapterPosition(openedPositions.keyAt(i));
...
//		isRtl = resources.getBoolean(R.bool.rtl_enabled);
	...
	{
		if (translationX > 0f && ((!isRtl && holder.getStartHiddenViewSize() == 0f) ^ (isRtl && holder.getEndHiddenViewSize() == 0f)))
		{
			return true;
		} else if (translationX < 0f && ((!isRtl && holder.getEndHiddenViewSize() == 0f) ^ (isRtl && holder.getStartHiddenViewSize() == 0f)))
		{
			return true;
		}
		return false;
	...
	private void select(SwipeOpenViewHolder selected, int actionState)
	{
		...
		if (closeOnAction && openedPositions.size() > 0)
		{
			for (int i = 0; i < openedPositions.size(); i++)
			{
				...
				if (holder instanceof SwipeOpenViewHolder && (selected == null || holder.getAdapterPosition() != selected.getViewHolder().getAdapterPosition()))
				{
					closeOpenHolder((SwipeOpenViewHolder) holder);
				}
				openedPositions.removeAt(i);
		...
		if (this.selected != null)
		{
			...
			if (prevSelected.getViewHolder().itemView.getParent() != null)
			{
				...
				if (ViewCompat.getTranslationX(prevSelected.getSwipeView()) != 0 || ViewCompat.getTranslationY(prevSelected.getSwipeView()) != 0)
				{
					...
					final RecoverAnimation rv = new RecoverAnimation(prevSelected, prevActionState, currentTranslateX, currentTranslateY, targetTranslateX,
							targetTranslateY);
					final long duration = callback.getAnimationDuration(recyclerView, ANIMATION_TYPE_SWIPE, targetTranslateX - currentTranslateX,
		...
		if (selected != null)
		{
			selectedFlags = (callback.getAbsMovementFlags(recyclerView, selected.getViewHolder()) & actionStateMask) >> (this.actionState
					* DIRECTION_FLAG_COUNT);
			selectedStartX = selected.getViewHolder().itemView.getLeft() + ViewCompat.getTranslationX(selected.getSwipeView());
			selectedStartY = selected.getViewHolder().itemView.getTop() + ViewCompat.getTranslationY(selected.getSwipeView());
	...
	public void onChildViewAttachedToWindow(View view)
	{
		...
		if (openedPositions.get(holder.getAdapterPosition(), null) != null)
		{
			...
			if (recyclerView.getLayoutManager().canScrollVertically())
			{
				...
				ViewCompat.setTranslationX(swipeHolder.getSwipeView(), state == SavedOpenState.START_OPEN ? swipeHolder.getStartHiddenViewSize() * rtlFlipStart
						: swipeHolder.getEndHiddenViewSize() * rtlFlipEnd);
			} else
			{
				ViewCompat.setTranslationY(swipeHolder.getSwipeView(),
						state == SavedOpenState.START_OPEN ? swipeHolder.getStartHiddenViewSize() : swipeHolder.getEndHiddenViewSize() * -1);
			}
		}
	}
	...
	private void closeOpenHolder(SwipeOpenViewHolder holder)
	{
		...
		final long duration = callback.getAnimationDuration(recyclerView, ANIMATION_TYPE_SWIPE, translationX, translationY);
		rv.setDuration(duration);
		recoverAnimations.add(rv);
		rv.start();
	...
	private int checkPreviousSwipeDirection(RecyclerView.ViewHolder viewHolder)
	{
		...
		final int absoluteMovementFlags = callback.convertToAbsoluteDirection(originalMovementFlags, ViewCompat.getLayoutDirection(recyclerView));
	...
	public abstract static class Callback extends ItemTouchHelper.Callback
	{
		...
		private void onDraw(Canvas c, RecyclerView parent, SwipeOpenViewHolder selected, List<RecoverAnimation> recoverAnimationList, int actionState, float dX,
