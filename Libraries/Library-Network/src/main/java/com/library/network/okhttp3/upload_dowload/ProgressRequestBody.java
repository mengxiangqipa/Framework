package com.library.network.okhttp3.upload_dowload;
...
public class ProgressRequestBody extends RequestBody {
    ...
    public void writeTo(BufferedSink sink) throws IOException {
        ...
        // Okio.buffer(Okio.sink(((BufferedSink)writeTo1(sink)).outputStream()));
        // BufferedSink progressSink = Okio.buffer(writeTo1(sink));

        ProgressOutputStream progressOutputStream = new ProgressOutputStream(
                sink.outputStream(), progressListener, contentLength());

        BufferedSink progressSink = Okio
                .buffer(Okio.sink(progressOutputStream));
