...
public class OverScrollView extends ScrollView
{
	...
	Handler updateHandler = new Handler()
	{
		...
		public void handleMessage(Message msg)
		{
			...
			MOVE_SPEED = (float) (5 + 15 * Math.tan(Math.PI / 2 / getMeasuredHeight() * (pullDownY + Math.abs(pullUpY))));
			if (pullDownY > 0)
	...
	private boolean canPullDownFromUserSet = true;
	private boolean canPullUpFromUserSet = true;
	...
	public boolean dispatchTouchEvent(MotionEvent ev)
	{
		...
		case MotionEvent.ACTION_MOVE:
			...
			if (mEvents == 0)
			{
				if (canPullDown && isCanPullDown() && canPullDownFromUserSet)
				{
					// 可以下拉，正在加载时不能下拉
					// 对实际滑动距离做缩小，造成用力拉的感觉
				...
				} else if (canPullUp && isCanPullUp() && canPullUpFromUserSet)
				{
					// 可以上拉，正在刷新时不能上拉
			...
			lastY = ev.getY();
			// 根据下拉距离改变比例
			radio = (float) (2 + 3 * Math.tan(Math.PI / 2 / getMeasuredHeight() * (pullDownY + Math.abs(pullUpY))));
			requestLayout();
		...
		case MotionEvent.ACTION_UP:
			hide();
			overScrollTrigger();
			if (mOverScrollTinyListener != null && ((isCanPullDown() && canPullDownFromUserSet) || (isCanPullUp() && canPullUpFromUserSet)))
			{
	...
	protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec)
	{
		...
		if (getChildCount() > 0)
		{
			...
			if (targetSdkVersion >= Build.VERSION_CODES.M)
			{
				...
				heightPadding = 0 + lp.topMargin + lp.bottomMargin;
			} else
			{
				widthPadding = 0;
				heightPadding = 0;
