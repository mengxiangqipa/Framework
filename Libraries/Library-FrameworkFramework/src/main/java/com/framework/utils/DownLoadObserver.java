 ...
 *      在下载之后调用
 *     getContentResolver().registerContentObserver(Uri.parse("content://downloads/"), true, new DownLoadObserver(new Handler(),VodActivity.this,id));
 *     用了这个就可以不用广播监听完成了
 */
...
//		String name = PreferenceHelper.getStringData(DownLoadManagerUtils.DownLoad_FileName);
		...
		{
			mDownload_so_far = cursor.getInt(cursor.getColumnIndexOrThrow(DownloadManager.COLUMN_BYTES_DOWNLOADED_SO_FAR));
