		...
		if (!TextUtils.isEmpty(message))
		{
			FrameApplication.mHandler.post(new Runnable()
			{
				...
				public void run()
				{
					try
					{
						Toast.makeText(FrameApplication.frameApplication, message, toastLong ? Toast.LENGTH_LONG : Toast.LENGTH_SHORT).show();
					} catch (Exception e)
					{
						e.printStackTrace();
					}
	...
	public void showToast(@StringRes final int resId, final boolean toastLong)
	{
		FrameApplication.mHandler.post(new Runnable()
		{
			...
			public void run()
			{
				try
				{
					Toast.makeText(FrameApplication.frameApplication, FrameApplication.frameApplication.getResources().getString(resId),
