...
public class CustomFlowLayout extends LinearLayout
{
    ...
    protected void onLayout(boolean changed, int l, int t, int r, int b)
    {
        ...
        for (int i = 0; i < childCount; i++)
        {
            ...
            // 如果需要换行
            if (childWidth + lineWidth + lp.leftMargin + lp.rightMargin + getPaddingLeft() + getPaddingRight() > width)
            {//2016/11/18添加paddingLeft与paddingRight
                // 记录LineHeight
                ...
                //				Log.e("yy","自定义view lp.topMargin: " + lp.topMargin+ "    lp.bottomMargin:" + lp.bottomMargin);
                // 重置view的集合
    ...
    class CustomOnCheckedChangeListener implements CompoundButton.OnCheckedChangeListener
    {
        ...
        public void onCheckedChanged(CompoundButton compoundButton, boolean b)
        {
            if (compoundButton.getId() >= 10000 && compoundButton.getId() < 40000)
            {
                ...
                if ((getContext() instanceof OnCheckedChangeListener_ || onCheckedChangeListener_ != null) && compoundButton instanceof CheckBox)
                {
                ...
                } else if (b && (getContext() instanceof OnSingleCheckedChangeListener_ || onSingleCheckedChangeListener_ != null)
                        && compoundButton instanceof RadioButton)
                {
                    ...
                    if (getContext() instanceof OnSingleCheckedChangeListener_)
                    {
                        ...
                        ((OnSingleCheckedChangeListener_) getContext()).onSingleCheckedChanged_(position, true, compoundButton);
                    } else
                    {
