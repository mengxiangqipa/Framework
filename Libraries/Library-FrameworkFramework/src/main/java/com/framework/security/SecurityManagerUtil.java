...
public class SecurityManagerUtil
{
    ...
    public final void put(Context context, String keyInSharedPreferences, String value)
    {
        if (!TextUtils.isEmpty(value))
        {
            ...
            {
                PreferencesUtil.getInstance().putString(FrameConstant.AES_KEY, Base64Coder.encodeString(AesUtils.getInstance().generateKey()));
            }
            if (value.length() > 128)
            {
                ...
                {
                    String cookiesWithRSA = RSAmethodInRaw.rsaEncrypt(context, value.substring(i * 128, Math.min((i + 1) * 128, value.length())));
                    if (i > 0)
                        sb.append(",,,,");
                    sb.append(Base64Coder.encodeString(cookiesWithRSA));
                }
                PreferencesUtil.getInstance().putString(keyInSharedPreferences, AesUtils.getInstance().encrypt(Base64Coder.decodeString(PreferencesUtil.getInstance().getString(FrameConstant.AES_KEY)), sb.toString()));
            } else
            {
                String cookiesWithRSA = RSAmethodInRaw.rsaEncrypt(context, value);
                PreferencesUtil.getInstance().putString(keyInSharedPreferences, AesUtils.getInstance().encrypt(Base64Coder.decodeString(PreferencesUtil.getInstance().getString(FrameConstant.AES_KEY)), Base64Coder.encodeString(cookiesWithRSA)));
            }
        }
    }
    ...
    private final String decode(Context context, String str)
    {
        ...
        {
            String tm = AesUtils.getInstance().decrypt(Base64Coder.decodeString(PreferencesUtil.getInstance().getString(FrameConstant.AES_KEY)), str);
