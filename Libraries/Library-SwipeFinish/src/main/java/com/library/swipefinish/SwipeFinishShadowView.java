...
/**
 *     @author YobertJomi
 *     className SwipeFinishShadowView
 *     created at  2020/10/23  17:37
 */
class SwipeFinishShadowView extends FrameLayout {
    private static final String TAG = SwipeFinishShadowView.class.getSimpleName();
     ...
     * 阴影资源 id
     */
    private int mShadowResId = R.drawable.sfl_shadow;
    /**
     * 阴影区域的透明度是否根据滑动的距离渐变
     */
    ...
    SwipeFinishShadowView(Activity activity) {
        super(activity);
        mActivity = activity;
    ...
    void bindPreActivity() {
        ...
        if (mPreActivity == null) {
            Activity preActivity = SwipeFinishManager.getInstance().getPenultimateActivity(mActivity);
            if (preActivity != null) {
    ...
    void unBindPreActivity(boolean isNeedAddImageView) {
        ...
        if (mPreDecorView != null && mPreContentView != null) {
            ...
            if (!(mPreContentView instanceof SwipeFinishLayout)) {
    ...
    private boolean containsProblemView(ViewGroup viewGroup) {
        ...
        for (int i = 0; i < childCount; i++) {
            ...
            if (SwipeFinishManager.getInstance().isProblemView(childView)) {
                return true;
    ...
    private void onPanelSlide(Activity currentActivity, float slideOffset) {
        try {
            Activity preActivity = SwipeFinishManager.getInstance().getPenultimateActivity(currentActivity);
            if (preActivity != null) {
