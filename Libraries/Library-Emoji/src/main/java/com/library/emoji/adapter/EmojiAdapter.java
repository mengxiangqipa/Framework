...
public class EmojiAdapter extends UniversalAdapter<Integer>
{
    ...
    public EmojiAdapter(@NonNull Context context, @LayoutRes int layoutId, @Nullable List list, OnClickedEmoji onClickedEmoji)
    {
    ...
    protected void getItemView(UniversalViewHolder viewHolder, final Integer info, int position)
    {
        ...
        RelativeLayout.LayoutParams layoutParams = new RelativeLayout.LayoutParams((ScreenUtils.getInstance().getScreenWidthPx(context) - ScreenUtils.getInstance().dp2px(context, 20)) / 7, RelativeLayout.LayoutParams.WRAP_CONTENT);
